...
class Transformer_Role(nn.Module):
    def __init__(self, ac_weights, rl_weights, next_activity, dropout, d_model, d_hidden, n_size):
        ...
        self.ff_time = nn.Sequential(
            nn.Linear(4 * self.d_model, self.d_hidden), #todo check if right
            nn.ReLU(),
            nn.Dropout(dropout)
        )
    ...
    def forward(self, x_ac, x_rl, x_t):
        ...
        context = torch.sum(c_t, dim = 1) # (B, D_model * n)
        ...
        fft_in = torch.cat([c_t, t_embs], dim=-1)
        fft_in = torch.sum(fft_in, dim=1)
        fft_out = self.ff_time(fft_in)
        time_output = self.time_output(fft_out)

        return act_output, time_output, att_weights

def model_training_role(model, optimizer, train_data, val_data, test_data, MILESTONE_DIR, epochs=10, patience=15,
    ...
    for epoch in range(epochs):
        train_loss, train_acc, train_mae = train_loop_role(model, train_data, criterion, reg_share, optimizer, scheduler)
        val_loss, val_acc, val_mae, _ = test_loop_role(model, val_data, criterion)
    ...
    path = os.path.join(os.path.join(MILESTONE_DIR, 'models'), f'transformer_role_{N_SIZE}n_{reg_share}reg_{epoch+1}epochs.pt')
    torch.save(model.state_dict(), path)
    print(f'Model saved at {path}')

    return model

def model_evaluation_role(model, vec_test, args, y_scaler2):

    start = time.time()
    X_test, y_test_act, y_test_time = generate_inputs_role(vec_test, args)
    out_act, out_t, out_att = model(X_test[0].long(), X_test[1].long(), X_test[2])
...
def train_loop_role(model, train_data, criterion, reg_share, optimizer, scheduler=None):
    train_loss = 0.0
    train_acc = 0.0
    train_mae = 0.0
    num_correct = 0
...
def test_loop_role(model, test_data, criterion):
    test_loss = 0.0
    test_acc = 0.0
    test_mae = 0.0
    num_correct = 0
...
def generate_inputs_role(vec, args):
...
def get_dataloaders_role(vec_train, vec_test, args, batch_size=64):
    X_train, y_train_act, y_train_time = generate_inputs_role(vec_train, args)
    X_test, y_test_act, y_test_time = generate_inputs_role(vec_test, args)

    train_dataset = TensorDataset(X_train[0], X_train[1], X_train[2], y_train_act, y_train_time)
    ...
    test_dataset = TensorDataset(X_test[0], X_test[1], X_test[2], y_test_act, y_test_time)
...
def prepare_data_suff_role(test_path, args):
    ...
    if args['log_name'] is not 'bpic17':
        for i in range(len(test_df)):
            test_df.role = test_df.role.fillna('000')
            test_df.loc[i, 'role'] = 'role_' + str(test_df.loc[i, 'role'])


    return test_df

def create_pref_suff_role(df, ac_index, rl_index, DeM='All'):
    prefixes = list()
    ...
    for case in cases:
        ...
        if DeM == 'All':
            for i in range(0, len(trace) - 1):
                ac_pref.append(ac_index[trace[i]['task']])
                rl_pref.append(rl_index[trace[i]['role']])
                ...
                prefixes.append(dict(ac_pref=ac_pref.copy(),
                                    ...
                                    rl_suff=[rl_index[x['role']] for x in trace[i + 1:]],
                                    t_pref=t_pref.copy(),
    ...
    for x in prefixes:
        ...
        x['rl_suff'].append(rl_index['end']) #todo should become EOS for consistency
        x['rem_time'].append(0)

    return prefixes

def predict_beam_role(model, prefixes, max_trc_length, time_dim, index_ac, rl_index, scaler, beam_size=1):
    count = 0
    beam_size = beam_size if len(index_ac.keys()) >= beam_size else len(index_ac.keys()) - 1
    print(f'The max trace length = {max_trc_length}')
    for prefix in tqdm(prefixes):
        ...
        x_rl_n = np.append(np.zeros(time_dim),
        ...
        beam = [(1, x_ac_n, x_rl_n, x_t_n, ac_suf, t_suf)]
        completed = []

        for _ in range(max_trc_length):
            new_beam = []

            for score, x_ac_n, x_rl_n, x_t_n, ac_suf, t_suf in beam:
                a_inp = np.flip(x_ac_n, axis=1).copy()
                rl_inp = np.flip(x_rl_n, axis=1).copy()
                t_inp = np.flip(x_t_n, axis=1).copy()

                pred_act, pred_time, _ = model(x_ac=torch.tensor(a_inp), x_rl=torch.tensor(rl_inp), x_t=torch.tensor(t_inp))

                t_next = pred_time.detach().squeeze(0).squeeze(0).reshape(-1,1)#.item()

                t_next = scaler.inverse_transform(t_next)
                t_next = x_t_n[0][-1] + t_next[0][0]
                t_next = [[t_next]] #As we have timelapsed since start of case as input
                ...
                for i in range(beam_size):
                    ...
                    new_x_rl_n = np.append(x_rl_n, [[rl_index['unk']]], axis=1)
                    new_x_rl_n = np.delete(new_x_rl_n, 0, 1)
                    new_ac_suf = ac_suf + [pos]
                    new_t_suf = t_suf + [t_next[0][0]]

                    if index_ac[pos] == 'EOS':
                        completed.append((score * prob, new_ac_suf, new_t_suf))
                    else:
                        new_beam.append((score * prob, new_x_ac_n, new_x_rl_n, new_x_t_n, new_ac_suf, new_t_suf))
            ...
            if not completed:
                best = max(new_beam, key=lambda x: x[0])
                prefix['ac_suff_pred'] = best[4]
                prefix['t_suff_pred'] = best[5]
