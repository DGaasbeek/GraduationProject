...
import time
from sklearn.metrics import mean_absolute_error, precision_recall_fscore_support, accuracy_score, precision_score, recall_score, f1_score
...
class Transformer_Poac_Time(nn.Module):
    ...
    def forward(self, x_ac, x_rl, x_t):
        ...
        return poac_output, time_output, att_weights

def poac_time_model_training_role(model, optimizer, train_data, val_data, test_data, MILESTONE_DIR, epochs=10, patience=15,
    ...
    for epoch in range(epochs):
        train_loss, train_acc, train_MAE = poac_time_train_loop_role(model, train_data, criterion, optimizer, scheduler)
        val_loss, val_acc, val_MAE = poac_time_test_loop_role(model, val_data, criterion)
    ...
    path = os.path.join(os.path.join(MILESTONE_DIR, 'models'),
                        f'poac_time_transformer_role_{N_SIZE}n__{epoch + 1}epochs.pt')
    torch.save(model.state_dict(), path)
    print(f'Model saved at {path}')

    return model

def poac_time_model_evaluation_role(model, vec_test, args): #todo:

    start = time.time()
    X_test, y_test_poac, y_test_poac_time = generate_inputs_poac_time(vec_test, args)
    out_poac, out_time, out_att = model(X_test[0].long(), X_test[1].long(), X_test[2])

    out_poac = np.array(out_poac.argmax(1).detach()).reshape(-1,1)
    out_t = np.array(out_time.squeeze(-1).detach()).reshape(-1,1)
    poac_targets = np.array(y_test_poac.argmax(1).detach()).reshape(-1,1)
    t_targets = np.array(y_test_poac_time.squeeze(-1).detach()).reshape(-1,1)

    MAE = mean_absolute_error(out_t, t_targets)
    ...
    poac_tw_pred = np.zeros(len(out_poac)).reshape(-1,1)
    poac_tw_target = np.zeros(len(out_poac)).reshape(-1,1)
    tw = 4 #timewindow

    for i in range(len(out_poac)):
        if out_poac[i][0] == 1 and out_t[i][0] <= tw:
            poac_tw_pred[i][0] = 1
        else:
            poac_tw_pred[i][0] = 0

        if poac_targets[i][0] == 1 and t_targets[i][0] <= tw:
            poac_tw_target[i][0] = 1
        else:
            poac_tw_target[i][0] = 0

    accuracy_ptw = accuracy_score(poac_tw_pred, poac_tw_target)
    precision_ptw = precision_score(poac_tw_pred, poac_tw_target)
    recall_ptw = recall_score(poac_tw_pred, poac_tw_target)
    f1_ptw = f1_score(poac_tw_pred, poac_tw_target)
    ...
    print(f"MAE: {MAE:.4f}")
    print(f"Accuracy: {accuracy_ptw:.4f}, precision ptw: {precision_ptw:.4f}, recall ptw: {recall_ptw:.4f}, f1: {f1_ptw:.4f}")
    print(f"Inference time: {inference_time:.4f}")


def poac_time_train_loop_role(model, train_data, criterion, optimizer, scheduler=None, reg_share=0.5):
    train_loss = 0.0
    train_acc = 0.0
    train_mae = 0.0
    num_correct = 0
    ...
    for batch in tqdm(train_data):
        ...
        x_ac, x_rl, x_t, y_poac, y_poac_time = batch
        # x_ac, x_rl, x_t, y_act, y_time = x_ac.to(device), x_rl.to(device), x_t.to(device), y_act.cuda().to(device), y_time.to(device)

        # Forward pass
        output_poac, output_time, _ = model(x_ac=x_ac.long(), x_rl=x_rl.long(), x_t=x_t)

        # Find the loss
        loss_poac = criterion(output_poac, y_poac.argmax(1))
        loss_time = crit_reg(output_time, y_poac_time.unsqueeze(-1))

        loss = (1-reg_share) * loss_poac + reg_share * loss_time
        # Backpropagate
        loss_poac.backward()
        optimizer.step()
...
def poac_time_test_loop_role(model, test_data, criterion):
    test_loss = 0.0
    test_acc = 0.0
    test_mae = 0.0
    num_correct = 0
...
def generate_inputs_poac_time(vec, args):
    ...
    y_poac_time = torch.tensor(vec['poac_time'])

    return x, y_poac, y_poac_time

def get_dataloaders_poac_time(vec_train, vec_test, args, batch_size=64):
    X_train, y_train_poac, y_train_poac_time = generate_inputs_poac_time(vec_train, args)
    X_test, y_test_poac, y_test_poac_time = generate_inputs_poac_time(vec_test, args)

    train_dataset = TensorDataset(X_train[0], X_train[1], X_train[2], y_train_poac, y_train_poac_time)
    ...
    test_dataset = TensorDataset(X_test[0], X_test[1], X_test[2], y_test_poac, y_test_poac_time)
